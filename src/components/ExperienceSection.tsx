...
const EXPERIENCE_DATA: ExperienceItem[] = [
  {
    ...
    description:
      'Owned the Contracts & Agreements platform processing millions of records for multi-country workflows. Built event-driven systems on AWS with DynamoDB, SQS, and Lambda. Automated incident triage from 20 minutes to under 5 seconds. Co-led Collective Bargaining Agreements launch for Italy.',
  ...
  },
  {
    ...
    description:
      'Led teams and worked with 3 clients to engineer AI agents: clienteling agent for sales automation, RAG-based VSA agent with Azure AI Search, voice customer service agent with Gemini Live API, and internal autonomous Jira resolution agent for DevOps workflows.',
  ...
  },
  {
    ...
    description:
      'Developed ETL pipeline consolidating data from five clinical studies (HDRS, BPRS, PANSS scales) into SQLite with Tkinter GUI, then built a comprehensive EMR system using PostgreSQL, Docker, Django, and Vite.js centralizing clinical research data management for CCNP.',
  ...
  },
  {
    ...
    description:
      'Conducted ML-driven AR/VR research for Quest by building a Unity package with 3D interaction techniques using UPM and the XR Interaction Toolkit, and leveraged Microsoft.Extensions.AI and Ollama LLMs in .NET microservices to power an AI-driven mind-mapping tool.',
...
const ExperienceItem = React.memo<{
  ...
  return (
    <div
      className="group pr-6 md:p-6 bg-background md:hover:bg-hover-bg cursor-pointer flex flex-col md:flex-row"
      onClick={handleClick}
    >
      <div className="w-full md:w-1/4 text-xs md:text-sm font-light text-muted-secondary mb-2 md:mb-0">
        {experience.dateRange}
      </div>
      <div className="w-full md:w-3/4">
        <h3 className="text-sm md:text-lg font-light mb-2 text-foreground flex flex-col md:flex-row md:items-center">
          <span className="mb-1 md:mb-0">{experience.title}</span>
          <span className="hidden md:inline mx-2">•</span>
          <span className="text-xs md:text-lg text-muted md:text-foreground">
            {experience.company}
          </span>
          <svg
        ...
        <p className="text-xs md:text-base font-light mb-4 text-muted">{experience.description}</p>
        <div className="flex flex-wrap gap-2">
          {experience.skills.map((skill, skillIndex) => (
            <span
              key={skillIndex}
              className="px-2 md:px-3 py-1 bg-hover-bg text-foreground text-xs md:text-sm rounded-full"
            >
              {skill}
            </span>
          ))}
...
const ExperienceSection: React.FC = () => {
  ...
  if (!isClient) {
    return (
      <div className="w-full h-full flex flex-col md:max-w-[50vw] bg-background text-foreground">
        <h2 className="text-lg uppercase md:normal-case md:text-3xl mb-8 font-bold md:font-light">
          Experience
        </h2>
        <div
          className="flex-grow md:overflow-y-auto md:pl-4 md:custom-scrollbar"
          style={{ direction: 'rtl' }}
        >
          <div style={{ direction: 'ltr' }}>
            <div className="space-y-12 md:space-y-0">
              ...
              <div className="animate-pulse">
                <div className="h-4 bg-hover-bg rounded mb-2"></div>
                <div className="h-3 bg-hover-bg rounded mb-4"></div>
                <div className="h-2 bg-hover-bg rounded"></div>
              </div>
            </div>
          </div>
        </div>
